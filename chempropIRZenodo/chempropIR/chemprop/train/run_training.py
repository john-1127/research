...
def run_training(args: Namespace, logger: Logger = None) -> List[float]:
    ...
    if args.normalization_start is not None or args.normalization_end is not None:
        if args.normalization_end is not None:
            assert args.normalization_end < args.num_tasks, (
                "Specified normalization end position is higher than the number of targets"
            )
        ...
        if args.normalization_start > args.normalization_end:
            args.normalization_start, args.normalization_end = (
    ...
    if args.save_smiles_splits:
        ...
        for dataset, name in [
            ...
            if args.features_path is not None:
                with open(
                    os.path.join(args.save_dir, name + "_features.csv"), "w"
                ) as f:
                    writer = csv.writer(f)
    ...
    if args.dataset_type == "spectra":
        ...
        norm_train_targets = pre_normalize_targets(
            train_targets,
            threshold=args.sm_thresh,
            torch_device=args.device,
            batch_size=args.batch_size,
        )
        norm_val_targets = pre_normalize_targets(
            val_targets,
            threshold=args.sm_thresh,
            torch_device=args.device,
            batch_size=args.batch_size,
    ...
    if args.dataset_type == "spectra":
        norm_test_targets = pre_normalize_targets(
            test_targets, torch_device=args.device, batch_size=args.batch_size
        )
    ...
    for model_idx in range(args.ensemble_size):
        ...
        if args.spectral_mask_path is not None or hasattr(model, "spectral_mask"):
            ...
            if model_idx == 0:
                masked_train_targets = apply_spectral_mask(
                    model.spectral_mask,
                    train_data.targets(),
                    train_data.features(),
                    args.device,
                ...
                masked_val_targets = apply_spectral_mask(
                    model.spectral_mask,
                    val_data.targets(),
                    val_data.features(),
                    args.device,
                ...
                masked_test_targets = apply_spectral_mask(
                    model.spectral_mask,
                    test_data.targets(),
                    test_data.features(),
                    args.device,
        ...
        for epoch in trange(args.epochs):
            ...
            if best_epoch + args.convergence_margin < epoch:
                info(
                    f"Model {model_idx} considered converged at epoch {epoch} based on convergence margin {args.convergence_margin}"
                )
                break
        ...
        model = load_checkpoint(
            os.path.join(save_dir, "model.pt"), cuda=args.cuda, logger=logger
        )
