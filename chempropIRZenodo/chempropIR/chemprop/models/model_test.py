...
class MoleculeModel(nn.Module):
    ...
    def create_ffn(self, args: Namespace):
        ...
        if args.ffn_num_layers == 1:
            ffn = [dropout, nn.Linear(first_linear_dim, args.output_size, bias=bias)]
        else:
            ffn = [
                dropout,
            ...
            for _ in range(args.ffn_num_layers - 2):
                ffn.extend(
                    [
                        ...
                        dropout,
                        nn.Linear(
                            args.ffn_hidden_size, args.ffn_hidden_size, bias=bias
                        ),
                    ]
                )
            ffn.extend(
    ...
    # TODO: autoencoder implement
    def create_autoencoder(self, args: Namespace):
        bias = args.bias_ffn
        self.output_activation = args.output_activation
        self.norm_range = args.norm_range
        self.multiclass = args.dataset_type == "multiclass"
        ...
        # Encoder
        encoder_layers = []
        in_dim = first_linear_dim
        for out_dim in args.autoencoder_encoder_dims:
            encoder_layers.append(nn.Linear(in_dim, out_dim, bias=bias))
        ...
        # Latent Space
        self.latent_layer = nn.Linear(in_dim, args.autoencoder_latent_dim, bias=bias)

        # Decoder
        decoder_layers = []
        in_dim = args.autoencoder_latent_dim
        for out_dim in reversed(args.autoencoder_encoder_dims):
            decoder_layers.append(nn.Linear(in_dim, out_dim, bias=bias))
            decoder_layers.append(activation)
            decoder_layers.append(dropout)
            in_dim = out_dim

        self.decoder_layers = nn.Sequential(*decoder_layers)
        self.decoder_output = nn.Linear(in_dim, args.output_size, bias=bias)

    # TODO: edit for autoencoder
...
def build_model(args: Namespace) -> nn.Module:
    ...
    # TODO: add args, parsing.py edit frzn_mpn_checkpoint and update_checkpoint_args
    args.use_autoencoder = True
    args.autoencoder_encoder_dims = [2048, 1024, 512, 256, 128]
    args.autoencoder_latent_dim = 64
