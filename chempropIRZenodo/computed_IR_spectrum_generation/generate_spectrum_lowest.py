...
from create_spectrum import set_x_spacing, fixed_var_spectrum, norm_integration
...
def create_spectrum(freq_logs, input_type, xs, norm_min, norm_max, fixed_var):
    gs = []
    spectra = None
    Gmin = 100000
    for freq_log in freq_logs:
        ...
        if log.G < Gmin:
            spectra = fixed_var_spectrum(log.wavenum, log.ir_intensities, xs, fixed_var)
            Gmin = log.G

    spectra_final = norm_integration(spectra, xs, norm_min, norm_max).tolist()
...
def search_dir(dir):
    #if os.path.isfile('{}_lowest.json'.format(dir)):
    #    return
    ...
    with open('{}_lowest.json'.format(dir), 'w') as output:
